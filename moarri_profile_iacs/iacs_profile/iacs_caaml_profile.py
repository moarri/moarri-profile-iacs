...
class SnowHeightComponents(namedtuple('SnowHeightComponents', ('components_type', 'snow_height', 'snow_height_uom', 'swe', 'swe_uom'))):

    @classmethod
    def create_default(cls, snow_height):
        return SnowHeightComponents(SnowHeightComponentsType.SNOW_HEIGHT, snow_height, IACSUnitsLengthType.CM, None, None)
...
class LayerInProfileBase(namedtuple('LayerInProfileBase', LayerPointObsRoot._fields+('depth_top', 'depth_top_uom', 'thickness', 'thickness_uom'))):
...
class StratigraphicLayerBase(namedtuple('StratigraphicLayerBase', LayerInProfileBase._fields+('validFormationTime', ))):

    @classmethod
    def create_default(cls, depth_top, thickness, validFormationTime):
        return StratigraphicLayerBase(depth_top, IACSUnitsLengthType.CM, thickness, IACSUnitsLengthType.CM, validFormationTime)
...
class GrainSize(namedtuple('GrainSize', ['uom', 'avg', 'avg_max'])):
    ...
    def create_default(cls, avg, avg_max):
        return GrainSize(IACSUnitsLengthType.MM, GrainSizeComponent.create_numeric(avg), GrainSizeComponent.create_numeric(avg_max))
...
class StratProfileLayer(namedtuple('StratProfileLayer', StratigraphicLayerBase._fields + ('grain_form_primary', 'grain_form_secondary', 'grain_size', 'hardness', 'lwc'))):

    @classmethod
    def create_default(cls, depth_top, thickness, valid_formation_time, grain_form_primary, grain_form_secondary, grain_size, hardness, lwc):
        return StratProfileLayer(depth_top, IACSUnitsLengthType.CM, thickness, IACSUnitsLengthType.CM, valid_formation_time, grain_form_primary, grain_form_secondary, grain_size, hardness, lwc)
